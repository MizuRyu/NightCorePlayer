...
final class MusicKitClientMock: MusicKitClient, @unchecked Sendable {
    ...
    func searchCatalogSongs(term: String, limit: Int, offset _: Int) async throws -> [Song] {
    ...
    func fetchRecentlyPlayedSongs(limit: Int) async throws -> [Song] {
        ...
        if let e = recentlyPlayedError {
            throw e
        }
        return Array(recentlyPlayedSongs.prefix(limit))
    }

    func fetchSimilarArtists(artist _: Artist) async throws -> [Artist] {
        fetchSimilarArtistsCalls += 1
        if let e = similarArtistsError {
            throw e
        }
