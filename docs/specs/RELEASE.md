...
注意: バージョンの「リリース方式」は提出をキャンセルして再提出すると
**「承認後に自動リリース」へ戻ることがある**（1.1 で実際に発生し、手動リリースのつもりが
承認と同時に全ユーザーへ即時公開された）。「審査へ提出」を押す直前に、
バージョンページの App Store バージョンのリリース欄と段階的リリースの設定を必ず見直す。
