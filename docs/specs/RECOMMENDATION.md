...
- C が不足するときは D を広げて総数を守る (逆も同様)。両方不足なら、あるだけ返す
- 端数は切り捨て (limit 20 → D 6)。`defaultLimit` は設計どおり 20 に固定
...
- 日次生成: 同日・同 limit はメモリキャッシュ (actor 内) を再利用。アプリ再起動で再生成
- 同時初回呼び出しは single-flight で1回の生成に合流させる
- 素材系 API が失敗したまま組んだ縮退結果はキャッシュしない (復旧後に再試行させる)
...
4. 未認証でもローカル履歴があれば投げずに C 枠だけで生成する (履歴も空なら従来どおり throw)

API 失敗は os.Logger (category: Recommendation) に警告として残す。キャンセルは途中打ち切り (結果はキャッシュしない)。
